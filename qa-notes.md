...
17. [Жизненный цикл ПО](#жизненный-цикл-по)
18. [Time to market](#time-to-market-ttm)
19. [Организационная структура управления](#организационная-структура-управления)
20. [Agile, Scrum & Kanban](#agile-scrum-и-kanban)
...
![Жизненный цикл](./assets/images/life_cycle.png)
...
**Жизненный цикл ПО** - период времени, который начинается с момента принятия решения о необходимости создания программного продукта и заканчивается в момент его полного изъятия из эксплуатации. 
      ...
      Включает в себя все этапы его развития: от возникновения потребности в нем до полного прекращения его использования вследствие морального старения или потери необходимости решения соответствующих задач.

![Жизненный цикл ПО](./assets/images/life_cycle_software.png)
...
4. Разработка.
5. Тестирование.
6. Развертывание - выход в продакшен / выход в эксплуатацию.
7. Эксплуатация и поддержка.
8. Закрытие. 

Это пример большого IT-продукта, но тот же подход применяется и к маленьким: например, скрипту или лендингу. При этом можно пропускать этапы, например:
...
Данный цикл разработки ( см. скрин ) называется **каскадной** или **водопадной** моделью.
Это название отражает суть модели, потому что каждый следующий этап начинается после предыдущего: нельзя сначала протестировать приложение, а потом придумать его дизайн.

      Водопадная модель - первая модель, появившаяся во время рзвития компьютерной эры. Раньше она удовлетворяла все потребности, но в современном мире она не так актуальна, так как имеет существенный недостаток -> чтобы построить качественный продукт, то потребуется потратить много времени и ресурсов. Главная проблема заключается в том, что мы не можем вернуться на предыдущий этап в рамках текущего цикла. Если ошибка в требованиях или дизайне обнаружилась только на этапе тестирования, команда не может просто откатиться назад и быстро что-то поправить. Единственный способ исправить такую ошибку — полностью завершить текущий цикл разработки, а затем начать весь процесс с самого начала (с пересмотра требований). 

--- 

## Time-to-market (TTM).

**TTM** - время от начала разработки идеи до ее конечной реализации, когда вы продаете ее клиенту; **Это время до выхода на рынок**.

![TTM](./assets/images/TTM.png)

**Чем меньше значение ТТМ, тем лучше.**

> Если создание и запуск продукта затягиваются, то компания может отстать от конкурентов и потерять выгоду, а само решение (продукт) - потерять свою инновационность.

Чтобы не затягивать тестирование и выход продукта на рынок, любой компании важно улучшать процессы тестирования:
...
![Соотношение цена/качество/скорость](./assets/images/ttm_qsp.png)

---

## Организационная структура управления.

**Организационная структура** - модель, формирующая устройство внутри компании. Схематично показывает направления работы компании, взаимосвязь сотрудников и распределение ответственности, прав и обязанностей.
...
Существует несколько видов логической структуры:
...
![Линейно-функциональная структура](./assets/images/line_func_str.png)

      В данной структуре мы видим, что сотрудники выполняют свою работу и отдают обратную связь своему начальнику — руководителю своего отдела, который в свою очередь передает информацию своему начальнику — руководителю разработки.
      
---

## Проектная структура.

**Проектная структура** - основана на приципе разделения компании на отдельные проекты, а не на отделы.

![Проектная структура](./assets/images/project_str.png)


         В компаниях с такой структурой есть отдельная должность - "менеджер" или "руководитель проекта", который ведет проект и отвечает за связь между сотрудниками проекта (аналитики, разработчики, тестировщики..). За координацию "Менеджеров" отвечает "Проектный офис (PMO)"
---

## Матричная структура.

**Матричная структура** - основана на принципе двойного подчинения исполнителей. В случае матричной структуры управления сотрудник компании подчиняется руководителю своего отдела и руководителю проекта.

![Матричная структура](./assets/images/matrix_str.png)

      В такой структуре за проектом закреплен как правило конкретный сотрудник, при этом работа может вестись как за одним проектом, так и за несколькими одновременно.

Здесь так же есть "менеджер проекта" и свои преймущетсва: 
...
## Кросс-функциональная структура / Продуктовый подход.

![Кросс-функциональная структура](./assets/images/x-func_str.png)

**Кросс-функциональная структура** - основана на приципе разделения компании на "отдельные продукты". По сути, это та же "проектная структура", но имеет ряд отличий:

| Критерий | Проектная структура | Продуктовая структура (кросс-функциональная команда) |
| :--- | :--- | :--- |
| **Длительность** | Проект временный (есть старт и финиш). | Продукт существует постоянно (в течение всего жизненного цикла). |
| **Цель** | Сдать проект в срок и в бюджет (дедлайн). | Создать долгосрочную ценность для пользователя. |
| **Команда** | Собирается под проект и распускается после сдачи. | Работает над продуктом непрерывно, без роспуска. |
| **Фокус менеджера** | Менеджер проекта (PM) — контроль сроков и бюджета. | Менеджер продукта (Product Manager) — стратегия развития и удовлетворенность клиента. |
| **Приоритеты** | Выполнение плановых задач. | Быстрая реакция на изменения рынка и обратную связь пользователей. |

      ПРИМЕР:

      *   Проект — создать лендинг для распродажи «Черная пятница» (разовый, временный).  
      *   Продукт — мобильное приложение банка (работает постоянно, требует обновлений и тестирования).

---

## Agile, Scrum и Kanban.

**AGILE** - Это подход к управлению проектами, предполагающий разбивку проекта
на этапы, а также непрерывное сотрудничество и совершенствование.

**SCRUM** - методика организации совместного рабочего процесса, в основе которой - поэтапная разработка и совершенствование продукта небольшой командой специалистов различного уровня, основной целью которой является дать желаемый результат заказчику вовремя и с минимально возможными затратами.

**SCRUM** относится к семейству **AGILE**-методик.
...
**Спринт** - период, в течении которого команда создает и совершенствует отдельную часть продукта, чтобы быть готовым к изменениям условий и исключить глобальную доработку.

Во время **спринтов** команда планирует задачи и проводит оценку предстоящей работы, и только после этого приступает к работе. Обычно длится от 1 до 3 недель.

**Kanban** — это система организации работы, которая фокусируется на **непрерывном потоке** задач без деления на жёсткие спринты. Она родилась на заводах Toyota, но сейчас активно используется в IT.

**Ключевые принципы Kanban:**
*   **Визуализация работы:** Все задачи отображаются на Kanban-доске (например, «Нужно сделать» → «В работе» → «На проверке» → «Готово»).
*   **Ограничение «работы в процессе» (WIP — Work In Progress):** Команда может взять только определённое количество задач одновременно. Это помогает не распыляться и быстрее доводить задачи до конца.
*   **Непрерывная доставка:** Нет фиксированных спринтов, задачи берутся в работу по мере поступления приоритетов от заказчика.

## Scrum & Kanban: Главные отличия

| Характеристика | Scrum | Kanban |
| :--- | :--- | :--- |
| **Итерации** | Жёсткие рамки спринтов (1-4 недели) | Непрерывный поток (без спринтов) |
| **Роли** | Чёткие роли (PO, SM, Team) | Жёстких ролей нет |
| **Приоритеты** | Приоритеты утверждаются до спринта | Приоритеты можно менять в любой момент |
| **Метрики** | Скорость команды (Velocity) | Среднее время выполнения задачи (Lead / Cycle Time) |
| **Подходит для** | Проектов с долгосрочным планированием и сложным бэклогом | Поддержки, эксплуатации и задач с часто меняющимися приоритетами |
...
* Использование TMS - ускоряют процесс, позволяя легко находить нужные ТС и быстро проставлять статусы.
* Выбор значимых значений и авто-тесты.
* Пирамида тестировния.
* Сдвиг тестирования влево (shift-left) - стратегия, при которой тестировщики подключаются к работе очень рано, что позволяет находить ошибки на более ранних этапах, что экономит и время и деньги при разработке.
* Командная работа на каждом этапе.
* Использоование линтеров.
* Непрерывная интеграция и деплой - есть тип ошибок - *ошибки конфигурации*. Справиться с ними помогает *DOCKER* - инструмент непрерывной интеграции и деплоя.


## Автоматизация тестирования.

**АТ** - метод , который выполняется с использоваием спец. программ.
...
* ТС, которыми проверяются самые критичные функции.
* ТС, которые регулярно повторяются.
* ТС, которые очень сложны и заниают много времени для выполнения вручную.

---

## Система тестирования.

**Система тестирования** - способ организации тестов в процессе разработки. Она включает в себя определенный порядок выполнения тестов и их место в процессе разработки. Количество тестов на каждом уровне зависит от конкретного проекта и его требований.

**Уровни системы тестирования:**

* #### Модульное тестирование 

МТ ищет дефекты в отдельных модулях приложения — объектах, классах, функциях, программных модулях и других компонентах, которые могут быть протестированы независимо друг от друга; Проверяем фрагменты кода.

Проводится с помощию юнит-тестов.

      
      Пример: 
      
       Нам нужно протестировать корзину интернет-магазина. 
       Работу этой корзины обеспечивает код из разных модулей — в том числе в нем есть функция, которая подсчитывает общую стоимость заказа.
...
ИТ проверяет взаимодействие между отдельными модулями — объектами, классами, функциями, программными модулями и так далее. Другими словами, мы проверяем, как два фрагмента кода работают вместе.

      Пример:

         В МТ мы проверили функцию суммирования и API по отдельности, а сейчас протестируем их вместе:
         
            * Правильно ли API отдает параметры в функцию
            * Верно ли API передает результат функции дальше

            В контексте системы тестирования термин «интеграционное тестирование» означает проверку взаимодействий модулей.
            Иногда этим термином обозначают проверку интеграции между двумя подсистемами, то есть какими-то большими частями программы.


* #### Системное тестирование

СТ выполняется для всей системы и проверяет функциональные и нефункциональные требования.

      В примере с корзиной мы зайдем на сайт, добавим несколько товаров в корзину и проверим, как отображается сумма заказа.
      Если сумма верна и видна в интерфейсе, то — функциональный системный тест пройден
